...
DL_EXPORT(void) initselect() SEG_SELECTMODULE_C;
static PyObject *select_select(PyObject *self, PyObject *args) SEG_SELECTMODULE_C;
static PyObject *set2list(fd_set *set, pylist fd2obj[FD_SETSIZE + 3]) SEG_SELECTMODULE_C;
static int list2set(PyObject *list, fd_set *set, pylist fd2obj[FD_SETSIZE + 3]) SEG_SELECTMODULE_C;
static void reap_obj( pylist fd2obj[FD_SETSIZE + 3] ) SEG_SELECTMODULE_C;
...
select_select(self, args)
	...
	PyObject *args;
{
#if defined(MS_WINDOWS) || defined(PALMOS)
...
#ifdef PALMOS
	else if (!PyArg_ParseTuple(tout, "ll", &tv.tv_sec, &tv.tv_usec)) {
		PyErr_SetString(PyExc_TypeError,
				"timeout must be a tuple (sec, usec) or None");
		return NULL;
	}
	else {
		tvp = &tv;
...
/* 	printf("before: max, ifdset, ofdset, efdset = %d %ld %ld %ld\n",  */
/* 	       max, ifdset, ofdset, efdset); */
...
/* 	printf("after: max, ifdset, ofdset, efdset, n = %d %ld %ld %ld %d\n",  */
/* 	       max, ifdset, ofdset, efdset, n); */
...
static PyMethodDef select_methods[] = {
    {"select",	select_select, 1, USE_DOC(select_doc)},
    {0,  	0},			     /* sentinel */
};
...
{
	PyObject *m, *d;
	m = Py_InitModule3("select", select_methods, USE_DOC(module_doc));
