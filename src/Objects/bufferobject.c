...
#include "other/bufferobject_c.h"

static PyObject *
_PyBuffer_FromMemory(base, ptr, size, flags)
	PyObject *base;
	void *ptr;
	int size;
	unsigned int flags;
	...
	int readonly;
{
	...
	/* if the base object is another non-base buffer, then "deref" it */
	if ( PyBuffer_Check(base) && !PyBuffer_IsBase((PyBufferObject*)base))
		base = ((PyBufferObject *)base)->b_base;

	flags = (readonly ? PyBuffer_READONLY : 0);
	return _PyBuffer_FromMemory(base, (char *)p + offset, size, flags);
}
...
PyObject *
PyBuffer_FromMemory(ptr, size, flags)
	void *ptr;
	int size;
	unsigned int flags;
{
	return _PyBuffer_FromMemory(NULL, ptr, size, flags | PyBuffer_READONLY);
}

PyObject *
PyBuffer_FromReadWriteMemory(ptr, size, flags)
	void *ptr;
	int size;
	unsigned int flags;
{
	...
	PyBufferObject *other;
{
	...
	if (!PyBuffer_IsValid(self)) {
		PyErr_SetString(PyExc_ValueError, "Buffer is no longer valid");
		return -1;
	}
...
{
	char buf[300];
	char *status = PyBuffer_IsReadOnly(self) ? "read-only" : "read-write";
