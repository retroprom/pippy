...
/* Forward */
static void initmain Py_PROTO((void)) SEG_PYTHONRUN_C;
static void initsite Py_PROTO((void)) SEG_PYTHONRUN_C;
static PyObject *run_err_node Py_PROTO((node *n, char *filename,
				   PyObject *globals, PyObject *locals)) SEG_PYTHONRUN_C;
static PyObject *run_node Py_PROTO((node *n, char *filename,
			       PyObject *globals, PyObject *locals)) SEG_PYTHONRUN_C;
static PyObject *run_pyc_file Py_PROTO((FILE *fp, char *filename,
				   PyObject *globals, PyObject *locals)) SEG_PYTHONRUN_C;
static void err_input Py_PROTO((perrdetail *)) SEG_PYTHONRUN_C;
static void initsigs Py_PROTO((void)) SEG_PYTHONRUN_C;
static void call_sys_exitfunc Py_PROTO((void)) SEG_PYTHONRUN_C;
static void call_ll_exitfuncs Py_PROTO((void)) SEG_PYTHONRUN_C;
...
void _gdb_hook(void);
	...
	DMESSAGE("starting Py_Initialize");

	if (initialized)
		return;
	initialized = 1;
	
	DMESSAGE("before getenvs");
	...
	DMESSAGE("before PyInterpreterState_New");
	interp = PyInterpreterState_New();
	if (interp == NULL)
		Py_FatalError("Py_Initialize: can't make first interpreter");

	DMESSAGE("before PyThreadState_New");
	tstate = PyThreadState_New(interp);
	if (tstate == NULL)
		Py_FatalError("Py_Initialize: can't make first thread");
	DMESSAGE("before PythreadState_Swap");
	(void) PyThreadState_Swap(tstate);

	DMESSAGE("before PyDict_New");
	interp->modules = PyDict_New();
	if (interp->modules == NULL)
		Py_FatalError("Py_Initialize: can't make modules dictionary");

	DMESSAGE("before _PyBuiltin_Init_1()");
	bimod = _PyBuiltin_Init_1();
	if (bimod == NULL)
		Py_FatalError("Py_Initialize: can't initialize __builtin__");
	DMESSAGE("before PyModule_GetDict");
	interp->builtins = PyModule_GetDict(bimod);
	Py_INCREF(interp->builtins);

	DMESSAGE("before _PySys_Init");
	sysmod = _PySys_Init();
	if (sysmod == NULL)
		Py_FatalError("Py_Initialize: can't initialize sys");
	DMESSAGE("before PyModule_GetDict(sysmod)");
	interp->sysdict = PyModule_GetDict(sysmod);
	Py_INCREF(interp->sysdict);
	DMESSAGE("before _PyImport_FixupExtension");
	_PyImport_FixupExtension("sys", "sys");
	DMESSAGE("before PySys_SetPath");
	PySys_SetPath(Py_GetPath());
	DMESSAGE("before PyDict_SetItemString");
	PyDict_SetItemString(interp->sysdict, "modules",
			     interp->modules);

	DMESSAGE("before _PyImport_Init");
	_PyImport_Init();

	/* phase 2 of builtins */
	DMESSAGE("before _PyBuiltin_Init_2");
	_PyBuiltin_Init_2(interp->builtins);
	DMESSAGE("before _PyImport_FixupExtension [2]");
	_PyImport_FixupExtension("__builtin__", "__builtin__");

	DMESSAGE("before initsigs");
	initsigs();  /* Signal handling stuff, including initintr() */

	DMESSAGE("before initmain");
...
Py_Finalize()
{
	...
	DMESSAGE("checking for exitfunc");
	call_sys_exitfunc();

	/* Get current thread state and interpreter pointer */
	DMESSAGE("getting threadstate");
	...
	int set_sys_last_vars;
{
	...
	if (f == NULL)
		...
		if (Py_FlushLine())
			PyErr_Clear();
		fflush(stdout);
		DMESSAGE("Starting traceback");
		err = PyTraceBack_Print(tb, f);
		DMESSAGE("Done traceback");
		if (err == 0 &&
		...
		if (err) {
			...
			printf("error occurred during traceback\n");
		}
