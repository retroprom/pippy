...
#endif
	{"getrefcount",	sys_getrefcount, 0, USE_DOC(getrefcount_doc)},
#ifdef USE_MALLOPT
	{"mdebug",	sys_mdebug, 0},
#endif
...
_PySys_Init()
{
	...
	m = Py_InitModule3("sys", sys_methods, USE_DOC(sys_doc));
	sysdict = PyModule_GetDict(m);

	DMESSAGE("sysmodule: <stdin>...");
...
{
	PyObject *v;
	DMESSAGE("starting SetPath");
#ifndef WITHOUT_GETPATH
	if ((v = makepathobject(path, DELIM)) == NULL)
		Py_FatalError("can't create sys.path");
	if (PySys_SetObject("path", v) != 0)
		Py_FatalError("can't assign sys.path");
	Py_DECREF(v);
#else /* !WITHOUT_GETPATH */
	v = NULL;
	DMESSAGE("setting object");
	if (PySys_SetObject("path", v) != 0)
		Py_FatalError("can't assign sys.path");
#endif /* !WITHOUT_GETPATH */
}
