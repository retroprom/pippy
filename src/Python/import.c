...
_PyImport_Init()
{
	 ...
	 */
#ifndef NO_DYNAMIC_LINK       
	for (scan = _PyImport_DynLoadFiletab; scan->suffix != NULL; ++scan)
		++countD;
#endif /* NO_DYNAMIC_LINK */
	...
	char *name;
{
	...
	DMESSAGE("Marshalling");
/* 	printf("begin marshalling %s\n", name); */
	co = PyMarshal_ReadObjectFromString((char *)p->code, size);
/* 	printf("end marshalling %s\n", name); */
	if (co == NULL) DMESSAGE("Marshalling failed");
	if (co == NULL)
		return -1;
	if (!PyCode_Check(co)) {
		DMESSAGE("OOPS - not a code object")
		Py_DECREF(co);
...
/* 	printf("done executing %s\n", name); */
	DMESSAGE("Done Executing");
	Py_DECREF(co);
	if (m == NULL) DMESSAGE("Execution failed");
	if (m == NULL)
		return -1;
	Py_DECREF(m);
	return 1;
...
DEF_DOC(doc_load_module,
"load_module(name, file, filename, (suffix, mode, type)) -> module\n\
Load a module, given information returned by find_module().\n\
The module name must include the full package name, if any.\
");

DEF_DOC(doc_get_magic,
"get_magic() -> string\n\
Return the magic number for .pyc or .pyo files.\
");

DEF_DOC(doc_get_suffixes,
"get_suffixes() -> [(suffix, mode, type), ...]\n\
Return a list of (suffix, mode, type) tuples describing the files\n\
that find_module() looks for.\
");

DEF_DOC(doc_new_module,
...
static PyMethodDef imp_methods[] = {
	{"find_module",		imp_find_module,	1, USE_DOC(doc_find_module)},
	{"get_magic",		imp_get_magic,		1, USE_DOC(doc_get_magic)},
	{"get_suffixes",	imp_get_suffixes,	1, USE_DOC(doc_get_suffixes)},
...
initimp()
{
	...
	m = Py_InitModule4("imp", imp_methods, USE_DOC(doc_imp),
