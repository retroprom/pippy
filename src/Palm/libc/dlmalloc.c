...
void get_allocations(long *allocated, long *deallocated)
{
  ...
  if (allocated != NULL)
    *allocated =  (unsigned long)(mi.uordblks + mi.hblkhd);
  if (deallocated != NULL)
    *deallocated = (mi.arena + mi.hblkhd)/*(mi.usmblks)*/;
}
...
/* Free and malloc_unlink the last region entry from the region list */
